...
use storm::{MssqlDelete, MssqlLoad, MssqlSave, Result, prelude::*};
...
fn provider() -> ProviderContainer {
    ...
    provider.register("", TransactionScoped::from(MssqlFactory(config)));

    provider
}

#[tokio::test]
async fn transaction_scoped() -> Result<()> {
